...
- **A Resources page** — ~35 vetted, link-checked external references grouped by subject area and tagged by track: where to register and schedule, deep links into the official Claude / Claude Code docs for each topic the exams test (tool use, prompt caching, batches, MCP, CLAUDE.md, hooks, subagents, models, prompting…), the claude.ai help center for the Associate track, and a couple of genuinely good community guides. No braindumps.
